...
if __name__ == '__main__':
    ...
    for symbol in SYMBOLS:
        ...
        for fromdate in date_range:
            ...
            # Quotes csv file
            fname = f'Q-{symbol}-{_from}.csv'  # for example, Q-AAPL-2016-01-04.csv
            ...
            quotes = api.polygon.historic_quotes_v2(
                symbol=symbol,
                date=_from,
                timestamp=start_timestamp_of_interest,
                timestamp_limit=end_timestamp_of_interest).df
                
            pull_length = len(quotes)   # The number of data points we got
            
            print('pull 1')

            while True:        
                lastTimestamp = quotes.iloc[len(quotes)-1,0]     
                ...
                quotesappend = api.polygon.historic_quotes_v2(
                    symbol=symbol,
                    date=_from,
                    timestamp=start_timestamp_of_interest,
                    timestamp_limit=end_timestamp_of_interest).df
                ...
                pull_length = len(quotesappend)
                
                quotes = quotes.append(quotesappend) 
                
            quotes.to_csv(full_name)  